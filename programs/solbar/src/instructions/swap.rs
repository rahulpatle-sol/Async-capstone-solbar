...
use anchor_spl::{associated_token::AssociatedToken, token_2022::Token2022, token_interface::{mint_to, Mint, MintTo, TokenAccount}};
...
pub fn swap_handler(ctx: Context<Swap>, sol_amount: u64) -> Result<()> {
    ...
    let token_amount = (sol_amount as u128)
        .checked_mul(scale as u128).ok_or(SolbarError::Overflow)?
        .checked_div(price as u128).ok_or(SolbarError::Overflow)? as u64;
    ...
    ctx.accounts.token_state.total_minted = ctx.accounts.token_state.total_minted
        .checked_add(token_amount).ok_or(SolbarError::Overflow)?;
    msg!("[Solbar] Swap! SOL:{} Tokens:{} User:{}", sol_amount, token_amount, ctx.accounts.user.key());
    Ok(())
}